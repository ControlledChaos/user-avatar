...
namespace User_Avatars\Classes\Users;
...
class User_Avatars {
	...
	public function capability() {
		...
		add_settings_field(
			...
			__( 'Avatar Upload Permission',	'user-avatars' ),
			[ $this, 'capability_field' ],
			'discussion',
			'avatars',
			[ esc_html__( 'Only allow users with file upload capabilities to upload local avatars (Authors and above).', 'user-avatars' ) ]
		);
	...
	public function user_avatar_form( $profileuser ) {
		...
		?>
		<h2><?php _e( 'User Avatar', 'user-avatars' ); ?></h2>

		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="basic-user-avatar"><?php _e( 'Upload or Delete', 'user-avatars' ); ?></label></th>
	...
	public function bbpress_user_profile() {
		...
		echo '<div>';
		echo '<label for="basic-local-avatar">' . __( 'Avatar', 'user-avatars' ) . '</label>';
	...
	public function avatar_defaults( $options = [] ) {
		...
		$options = [
			...
			$defaults['blank']   => __( 'Blank', 'user-avatars' )
		];
